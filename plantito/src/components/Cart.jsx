...
import { selectCartItems, selectCartState, selectTotalAmout, selectTotalQTY, setClearCartItem, setCloseCart, setGetTotals } from '../app/CartSlice.js'
...
const Cart = () => {
  ...
  const totalAmount = useSelector(selectTotalAmout);
        ...
        <div className={`fixed top-0 left-0 right-0 bottom-0 blur-effect-theme w-full h-screen opacity-100 z-[250] ${ifCartState ?  'opacity-100 visible translate-x-0':'opacity-0 invisible translate-x-8'}`}>
            <div className={`bg-white h-screen max-w-xl w-full absolute right-0`}>
                ...
                {cartItems?.length === 0 ? <CartEmpty onCartToggle={onCartToggle}/> : <div>
                  <div className='flex items-start justify-start flex-col gap-y-7 lg:gap-y-2 overflow-y-scroll h-[81vh] scroll-smooth scroll-hidden py-3 '>
                  ...
                  <div className='bottom-0 bg-white w-full px-5 py-2 grid items-center'>
                    ...
                    <div className='grid items-center gap-2'>
                      <p className='text-sm font-medium text-center'>Shipping will calculate at shipping fee</p>
