...
const ondecreaseItem = () => {
  ...
  return (
   <>
    <div className='bg-white flex items-center justify-between w-full px-5'>
      <div className='flex items-center gap-5'>
        <div className={`bg-gradient-to-b ${color} ${shadow} relative rounded p-3 hover:scale-105 duration-75 ease-in-out grid items-center`}>
          <img src={img} alt={`img/cart-item/${id}`} className='w-10 h-auto object-fill lg:w-28' />
        </div>
      ...
      <div className='grid items-center gap-5'>
        ...
        <div className='grid items-center justify-center'>
          <button type='button' className='bg-theme-cart rounded w-6 h-6 lg:w-5 lg:h-5 flex items-center justify-center active:scale-90 cursor-pointer' onClick={onRemoveItem}><TrashIcon className='w-5 h-5 lg:w-4 lg:h-4 text-white stroke-[2]'/></button>
        </div>
      </div>
    </div>
   </>
